...
namespace AC
{
	...
	public class RuntimeInventory : MonoBehaviour
	{
		...
		public List<InvItem> RemoveEmptySlots (List<InvItem> itemList)
		{
			...
			/*for (int i=0; i<itemList.Count; i++)
			{
				if (itemList[i] != null && itemList[i].canCarryMultiple && !itemList[i].useSeparateSlots && itemList[i].count > itemList[i].maxCount)
				{
					while (itemList[i].count > itemList[i].maxCount)
					{
						int surplus = itemList[i].count - itemList[i].maxCount;
						int newItemCount = Mathf.Clamp (surplus, 0, itemList[i].maxCount);

						InvItem newItem = new InvItem (itemList[i]);
